...
看板直读的利润宽表——Aggregate 预聚合，查询按 `tenant_id` 命中分桶。为支持**双时间口径**，加入 `date_type` 作为聚合键的一部分（同一天数据分别按两种口径各存一份，查询时按 `date_type` 过滤）：

```sql
CREATE TABLE fact_profit_sku_daily (
    tenant_id        BIGINT       NOT NULL,
    marketplace_id   INT          NOT NULL,
    date_type        TINYINT      NOT NULL COMMENT '时间口径: 1=财务口径(费用入账日) 2=订单口径(买家下单日)',
    stat_date        DATE         NOT NULL COMMENT '按 date_type 语义: 财务入账日 或 买家下单日',
...
AGGREGATE KEY(tenant_id, marketplace_id, date_type, stat_date, sku)
...
净利润 net_profit =
  ...
  − inbound_freight         ← 头程总费用 × (SKU销量 / 批次总销量)  按销量分摊
...
1. **时间口径（双口径统计）**：利润按两种口径分别统计，通过 `fact_profit_sku_daily.date_type` 区分，看板可切换：
   - **口径一 · 财务口径（`date_type=1`）**：以亚马逊 SP-API 财务接口（Finances / 结算报表）中**费用入账/计算的日期**（`fact_finance_event.event_date`）为准。贴合真实现金流与对账。
   - **口径二 · 订单口径（`date_type=2`）**：以**买家下单日期**（`fact_order_item.purchase_date`）为准。费用通过 `order_id` 回填到对应下单日。贴合销售业绩视角。
   - ELT 时对同一批数据分别按两种口径各聚合一份写入宽表。

2. **成本分摊**：`dim_product_cost` 用 **SCD2（生效日期区间）** 存历史采购价；**头程运费按商品销量（销售数量）分摊**——将一批头程总费用按各 SKU 的销售数量占比摊到每件商品，即 `单件头程 = 该批头程总费用 × (SKU销量 / 批次总销量)`。
...
1. **时间口径（双口径）**：利润同时按两种口径统计，看板可切换（详见 8.5）：
   - **财务口径**：以 SP-API 财务接口费用的入账/计算日期为准（贴合现金流与对账）。
   - **订单口径**：以买家下单日期为准（贴合销售业绩视角）。
2. **头程运费分摊**：**按商品销量（销售数量）分摊**——`单件头程 = 该批头程总费用 × (SKU销量 / 批次总销量)`。
